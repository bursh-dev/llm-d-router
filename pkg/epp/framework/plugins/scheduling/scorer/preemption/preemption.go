...
func PreemptionScorerFactory(name string, rawParameters *json.Decoder, handle fwkplugin.Handle) (fwkplugin.Plugin, error) {
	...
	return &PreemptionScorer{
		...
		dataKey:        attrmetrics.ScalarMetricDataKey(params.AttributeKey),
...
// Consumes declares the endpoint attribute this scorer reads so the data layer
// extracts it. The cumulative preemption counter is optional: a pod missing it
// scores neutral (1.0) rather than failing scheduling.
func (s *PreemptionScorer) Consumes() fwkplugin.DataDependencies {
...
func (s *PreemptionScorer) Score(ctx context.Context, _ *fwksched.InferenceRequest, endpoints []fwksched.Endpoint) map[fwksched.Endpoint]float64 {
	...
	for _, endpoint := range endpoints {
		key := endpoint.GetMetadata().ID.String()
		seen[key] = struct{}{}
...
func (s *PreemptionScorer) scoreEndpoint(endpoint fwksched.Endpoint, key string, now time.Time, logger logr.Logger) float64 {
	cur, ok := attrmetrics.ReadScalarMetricValue(endpoint, s.dataKey)
