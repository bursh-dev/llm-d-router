...
func newEndpoint(name string, counter *float64, kvUsage float64, updateTime time.Time) fwksched.Endpoint {
	...
	if counter != nil {
		attr.Put(defaultAttributeKey, attrmetrics.ScalarMetricValue(*counter))
	}
	meta := &fwkdl.EndpointMetadata{NamespacedName: k8stypes.NamespacedName{Name: name, Namespace: "default"}}
...
func TestScore_NilMetricsNeutral(t *testing.T) {
	s := newTestScorer()
	attr := fwkdl.NewAttributes()
	attr.Put(defaultAttributeKey, attrmetrics.ScalarMetricValue(3))
	meta := &fwkdl.EndpointMetadata{NamespacedName: k8stypes.NamespacedName{Name: "pod", Namespace: "default"}}
...
func TestFactory_Defaults(t *testing.T) {
	...
	assert.Equal(t, defaultAttributeKey, s.attributeKey)
...
func TestFactory_ParsesParameters(t *testing.T) {
	...
	assert.Equal(t, "preempt-x", s.attributeKey)
